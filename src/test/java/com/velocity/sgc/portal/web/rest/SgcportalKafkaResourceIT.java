package com.velocity.sgc.portal.web.rest;
...
import com.velocity.sgc.portal.config.KafkaProperties;
...
class SgcportalKafkaResourceIT {
    ...
    void setup() {
        ...
        SgcportalKafkaResource kafkaResource = new SgcportalKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/sgcportal-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/sgcportal-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
